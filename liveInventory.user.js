...
function wrapper(plugin_info) {
    ...
    function addItemToCount(item, countMap, incBy, moniker) {
        ...
        if (item[2] && item[2].resource && item[2].timedPowerupResource) {
            ...
            countMap[key].displayName = `${translations[item[2].timedPowerupResource.designation] || item[2].timedPowerupResource.designation}`;
            ...
            if (!countMap[key]) {
                ...
                countMap[key].displayName = `${translations[item[2].resource.resourceType] || item[2].resource.resourceType}`;
            }
            countMap[key].count += incBy;
            if (!countMap[key].details[keyLoc]) countMap[key].details[keyLoc] = 0;
            countMap[key].details[keyLoc] += incBy;
            ...
            if (!countMap[key]) {
                ...
                countMap[key].displayName = `${translations[item[2].resourceWithLevels.resourceType] || item[2].resourceWithLevels.resourceType}`;
            }
            countMap[key].count += incBy;
            if (!countMap[key].details[keyLoc]) countMap[key].details[keyLoc] = 0;
            countMap[key].details[keyLoc] += incBy;
            ...
            if (!countMap[key]) {
                ...
                countMap[key].displayName = `${translations[item[2].modResource.resourceType] || item[2].modResource.resourceType}`;
            }
            countMap[key].count += incBy;
            if (!countMap[key].details[keyLoc]) countMap[key].details[keyLoc] = 0;
            countMap[key].details[keyLoc] += incBy;
...
<td>${el.count}</td><td>${el._formattedDistance}</td>
<td>${el.capsules.join(', ')}</td>
    ...
    function getItemTableBody(orderBy, direction) {
        ...
        return thisPlugin.itemCount.map((el) => {
            return `<tr><td>${el.type}</td><td>${el.resourceRarity || ''}</td><td>${el.count}</td></tr>`;
        }).join('');
    }
